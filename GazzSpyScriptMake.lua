...
newButton("SaveAll", function()
	...
	if totalLogs == 0 then
		TextLabel.Text = "⚠ Chưa có log nào để lưu!"
		return
	end

	TextLabel.Text = "💾 Đang lưu..."
	...
	if savedCount > 0 then
		TextLabel.Text = string.format(
			"✅ Đã lưu %d file! (RE:%d | RF:%d)%s → GazzSpy/FuncStore/",
			savedCount, reCount, rfCount,
			failCount > 0 and (" | ❌ Lỗi:" .. failCount) or ""
		)
	else
		TextLabel.Text = "❌ Không lưu được file nào! (Lỗi: " .. tostring(failCount) .. ")"
	end
end)
...
newButton("Autoblock", function()
	...
	history = {}; excluding = {}
	TextLabel.Text = string.format(
		"%s Autoblock %s - Đã reset history",
		autoblock and "✅" or "❌",
		autoblock and "BẬT" or "TẮT"
	)
end)
