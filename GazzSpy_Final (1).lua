...
function onBarInput(input)
    -- Hỗ trợ cả chuột (PC) và Touch (Mobile)
    ...
    connections["drag"] = RunService.RenderStepped:Connect(function()
        ...
        local delta = cur - startMousePos
        local vp = workspace.CurrentCamera.ViewportSize
        local newX = math.clamp(startFramePos.X + delta.X, 0,
            vp.X - (sideClosed and 131 or Background.AbsoluteSize.X))
        local newY = math.clamp(startFramePos.Y + delta.Y, 0,
            vp.Y - (closed and 19 or Background.AbsoluteSize.Y) - GuiInset.Y)
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        ...
        -- Drag: kết nối trực tiếp qua TopBar (PC + Mobile)
        TopBar.InputBegan:Connect(function(input)
            onBarInput(input)
        end)
        -- Backup: UserInputService cho trường hợp touch ngoài vùng
        table.insert(connections, UserInputService.InputBegan:Connect(function(input)
            local mousePos = UserInputService:GetMouseLocation() - GuiInset
            -- Chỉ drag khi chạm vào TopBar
        ...
        -- backgroundUserInput replaced by direct TopBar drag above
        connectResize()
...
-- ════════════════════════════════════════════════
-- HƯỚNG DẪN — đặt ĐẦU TIÊN để dễ thấy
-- ════════════════════════════════════════════════
newButton("Huong Dan", function() return "Xem huong dan su dung chi tiet GazzSpy" end,
function()
    codebox:setRaw([[-- ===========================================
-- GAZZSPY - HUONG DAN SU DUNG
-- ===========================================

-- [CACH SU DUNG CO BAN]
-- 1. Script tu dong bat spy khi load
-- 2. Nhan vao remote ben trai de xem code
-- 3. Keo TopBar de di chuyen cua so
-- 4. Nhan "GazzSpy" tren TopBar de bat/tat spy

-- [CAC NUT CHUC NANG]

-- Huong Dan  : Hien thi huong dan nay
-- GazzSave   : Luu code remote dang chon → GazzSpy/<ten>.lua
-- Save All   : Luu TAT CA remote → GazzSpy/SaveAll_<stamp>.lua
-- FuncStore  : Luu Function Info → GazzSpy/FuncStore/<ten>.lua

-- Copy Code  : Copy code remote dang chon
-- Copy Remote: Copy duong dan remote (game.xxx.RemoteEvent)
-- Run Code   : Thuc thi lai remote voi args goc
-- Get Script : Lay script da goi remote
-- Func Info  : Xem chi tiet function da goi remote

-- Clr Logs   : Xoa toan bo danh sach remote da spy
-- Excl (i)   : An remote nay theo ID
-- Excl (n)   : An tat ca remote cung ten
-- Clr Excl   : Bo an, hien lai remote
-- Block (i)  : Chan remote nay khoi fire server (theo ID)
-- Block (n)  : Chan tat ca remote cung ten
-- Clr Block  : Bo chan, remote hoat dong lai

-- [MAU REMOTE TRONG DANH SACH]
-- Vang = RemoteEvent (FireServer)
-- Tim  = RemoteFunction (InvokeServer)

-- [LUU Y]
-- Thu muc: GazzSpy/ va GazzSpy/FuncStore/
-- Executor can ho tro: hookmetamethod, writefile
-- ==========================================]])
    TextLabel.Text = "Dang hien Huong Dan..."
end)

-- ════════════════════════════════════════════════
-- GAZZSAVE
-- ════════════════════════════════════════════════
newButton("GazzSave", function() return "Luu code remote dang chon vao GazzSpy/<ten>.lua" end,
function()
    if not (writefile and isfolder) then TextLabel.Text = "Executor khong ho tro writefile!"; return end
    if not selected then TextLabel.Text = "Chua chon remote!"; return end
    local code = codebox:getString()
    if not code or code == "" then TextLabel.Text = "Code trong, khong luu."; return end
    local rname = safeName(selected.Remote and selected.Remote.Name or "Unknown")
    xpcall(function()
        writefile(GAZZSPY_FOLDER.."/"..rname.."_"..getStamp()..".lua",
            "-- GazzSpy | Remote: "..rname.."

"..code)
        TextLabel.Text = "Da luu: "..rname..".lua"
    end, function(e) TextLabel.Text = "Loi: "..tostring(e) end)
end)

-- ════════════════════════════════════════════════
-- SAVE ALL
-- ════════════════════════════════════════════════
newButton("Save All", function() return "Luu tat ca remote da spy vao GazzSpy/SaveAll_<stamp>.lua" end,
function()
    if not (writefile and isfolder) then TextLabel.Text = "Executor khong ho tro writefile!"; return end
    if #logs == 0 then TextLabel.Text = "Chua co log nao!"; return end
    TextLabel.Text = "Dang luu "..#logs.." remotes..."
    spawn(function()
        local allCode = "-- GazzSpy | Save All Remotes
"
            .."-- Tong so: "..tostring(#logs).."
...
"
                ...
                allCode = allCode.."-- ["..tostring(idx).."] Loi: "..tostring(e).."
...
"
        ...
        xpcall(function()
            ...
            TextLabel.Text = "Da luu "..tostring(saved).."/"..tostring(#logs).." remotes!"
        end, function(e) TextLabel.Text = "Loi ghi file: "..tostring(e) end)
    end)
end)
...
-- FUNCSTORE
-- ════════════════════════════════════════════════
newButton("FuncStore", function() return "Luu Function Info → GazzSpy/FuncStore/" end,
function()
    if not (writefile and isfolder) then TextLabel.Text = "Executor khong ho tro writefile!"; return end
    if not selected then TextLabel.Text = "Chua chon remote!"; return end
    local func = selected and selected.Function
    if not func then TextLabel.Text = "Khong tim thay function!"; return end
    spawn(function()
        local funcInfo = ""
        if typeof(func) ~= "string" then
            local _, result = xpcall(function()
                ...
                return v2v({functionInfo = {
                    ...
                    script = {SourceScript = tostring(ss), CallingScript = tostring(selected.Source)}
                }})
            end, function(e) return "-- Loi: "..tostring(e) end)
            funcInfo = result or "-- Khong the gen"
        else funcInfo = func end
        local rname = safeName(selected.Remote and selected.Remote.Name or "Unknown")
        local fcontent = "-- GazzSpy FuncStore | "..rname.."

"..funcInfo
        xpcall(function()
            writefile(GAZZSPY_FUNCSTORE.."/"..rname.."_FuncInfo_"..getStamp()..".lua", fcontent)
            codebox:setRaw(fcontent)
            TextLabel.Text = "FuncStore: Da luu "..rname.."!"
        end, function(e) TextLabel.Text = "Loi FuncStore: "..tostring(e) end)
    end)
end)
...
-- CÁC CHỨC NĂNG CHÍNH
-- ════════════════════════════════════════════════
newButton("Copy Code", function() return "Copy code remote dang chon" end,
function()
    setclipboard(codebox:getString())
    TextLabel.Text = "Da copy code!"
end)

newButton("Copy Remote", function() return "Copy duong dan remote dang chon" end,
function()
    if selected and selected.Remote then
        setclipboard(v2s(selected.Remote))
        TextLabel.Text = "Da copy path!"
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Run Code", function() return "Thuc thi lai remote dang chon voi args goc" end,
function()
    local Remote = selected and selected.Remote
    if Remote then
        TextLabel.Text = "Dang thuc thi..."
        xpcall(function()
            local rv
            if Remote:IsA("RemoteEvent") or Remote:IsA("UnreliableRemoteEvent") then
            ...
            TextLabel.Text = "Thanh cong! "..tostring(v2s(rv))
        end, function(err) TextLabel.Text = "Loi: "..tostring(err) end)
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Get Script", function() return "Lay script da goi remote, copy vao clipboard" end,
function()
    if selected then
        if not selected.Source then selected.Source = rawget(getfenv(selected.Function),"script") end
        setclipboard(v2s(selected.Source))
        TextLabel.Text = "Da copy script!"
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Func Info", function() return "Xem thong tin chi tiet function da goi remote" end,
function()
    local func = selected and selected.Function
    if func then
...
"..selected.Function)
        TextLabel.Text = "Da tao Function Info!"
    else TextLabel.Text = "Khong tim thay function!" end
end)

newButton("Clr Logs", function() return "Xoa toan bo danh sach remote da spy" end,
function()
    clear(logs)
    ...
    codebox:setRaw("")
    selected = nil
    TextLabel.Text = "Da xoa logs!"
end)

newButton("Excl (i)", function() return "An remote nay theo ID - van hoat dong nhung khong log" end,
function()
    if selected then blacklist[OldDebugId(selected.Remote)] = true; TextLabel.Text = "Da an: "..tostring(selected.Name)
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Excl (n)", function() return "An tat ca remote cung ten" end,
function()
    if selected then blacklist[selected.Name] = true; TextLabel.Text = "Da an ten: "..tostring(selected.Name)
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Clr Excl", function() return "Xoa danh sach an, remote hien lai" end,
function() blacklist = {}; TextLabel.Text = "Da xoa danh sach an!" end)

newButton("Block (i)", function() return "Chan remote nay khoi fire server (theo ID)" end,
function()
    if selected then blocklist[OldDebugId(selected.Remote)] = true; TextLabel.Text = "Da chan: "..tostring(selected.Name)
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Block (n)", function() return "Chan tat ca remote cung ten" end,
function()
    if selected then blocklist[selected.Name] = true; TextLabel.Text = "Da chan ten: "..tostring(selected.Name)
    else TextLabel.Text = "Chua chon remote!" end
end)

newButton("Clr Block", function() return "Xoa danh sach chan, remote hoat dong lai" end,
function() blocklist = {}; TextLabel.Text = "Da xoa danh sach chan!" end)
