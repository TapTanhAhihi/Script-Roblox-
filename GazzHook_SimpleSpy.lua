--!native
-- ╔══════════════════════════════════════════╗
-- ║     SimpleSpy Beta - GazzHook Edition    ║
-- ║  + Drag menu mượt hơn (kéo TopBar)       ║
-- ║  + Nút Save Code → GazzHook/             ║
-- ║  + Nút Save All → Lưu tất cả logs        ║
-- ║  + Mục FuncStore → Lưu Function Info     ║
-- ╚══════════════════════════════════════════╝
...
-- ── GazzHook: Tạo thư mục khi khởi động ───────────────────────────────────
local GAZZHOOK_FOLDER = "GazzHook"
local GAZZHOOK_FUNC_FOLDER = "GazzHook//FuncStore"

xpcall(function()
    if isfolder and makefolder then
        if not isfolder(GAZZHOOK_FOLDER) then
            makefolder(GAZZHOOK_FOLDER)
        end
        if not isfolder(GAZZHOOK_FUNC_FOLDER) then
            makefolder(GAZZHOOK_FUNC_FOLDER)
        end
    end
end, function(err) warn("[GazzHook] Lỗi tạo thư mục: "..tostring(err)) end)
...
-- GazzHook label nhỏ trên TopBar
local GazzLabel = Create("TextLabel",{Parent = TopBar,BackgroundTransparency = 1,Position = UDim2.new(0, 65, 0, 0),Size = UDim2.new(0, 80, 0, 18),Font = Enum.Font.SourceSansBold,Text = "| GazzHook",TextColor3 = Color3.fromRGB(255, 180, 50),TextSize = 12,TextXAlignment = Enum.TextXAlignment.Left})
...
newButton("Exclude (n)", function() return "Click to exclude all remotes with this name." end,
function()
    if selected then blacklist[selected.Name] = true; TextLabel.Text = "Excluded!" end
end)

newButton("Clr Blacklist", function() return "Click to clear the blacklist." end,
function() blacklist = {}; TextLabel.Text = "Blacklist cleared!" end)

newButton("Block (i)", function() return "Click to stop this remote from firing." end,
function()
    if selected then blocklist[OldDebugId(selected.Remote)] = true; TextLabel.Text = "Excluded!" end
end)

newButton("Block (n)", function() return "Click to stop remotes with this name from firing." end,
function()
    if selected then blocklist[selected.Name] = true; TextLabel.Text = "Excluded!" end
end)

newButton("Clr Blocklist", function() return "Click to stop blocking remotes." end,
function() blocklist = {}; TextLabel.Text = "Blocklist cleared!" end)
...
newButton("Logcheckcaller", function()
    ...
    TextLabel.Text = ("[%s] Logcheckcaller toggled"):format(configs.logcheckcaller and "ENABLED" or "DISABLED")
end)
...
newButton("Join Discord", function() return "Joins The Simple Spy Discord" end,
function()
...
--- [GazzHook] Lưu code đang xem vào GazzHook/<RemoteName>_<stamp>.lua
newButton("GazzSave", function()
    return "[GazzHook] Lưu code remote đang chọn vào thư mục GazzHook"
end, function()
    if not (writefile and isfolder) then
        TextLabel.Text = "[GazzHook] Executor không hỗ trợ writefile!"
        return
    end
    if not selected then
        TextLabel.Text = "[GazzHook] Chưa chọn remote nào!"
        return
    end
    local code = codebox:getString()
    if not code or code == "" then
        TextLabel.Text = "[GazzHook] Code trống, không lưu."
        return
    end
    local remoteName = safeName(selected.Remote and selected.Remote.Name or "Unknown")
    local filename = GAZZHOOK_FOLDER .. "//" .. remoteName .. "_" .. getStamp() .. ".lua"
    xpcall(function()
        writefile(filename, "-- GazzHook Save | Remote: " .. tostring(remoteName) .. "\n\n" .. code)
        TextLabel.Text = "[GazzHook] Đã lưu: " .. remoteName .. ".lua"
    end, function(err)
        TextLabel.Text = "[GazzHook] Lỗi: " .. tostring(err)
    end)
end)

--- [GazzHook] Lưu TẤT CẢ code trong logs vào GazzHook/SaveAll_<stamp>.lua
newButton("Save All", function()
    return "[GazzHook] Lưu tất cả remote đã spy vào GazzHook/SaveAll.lua"
end, function()
    if not (writefile and isfolder) then
        TextLabel.Text = "[GazzHook] Executor không hỗ trợ writefile!"
        return
    ...
    if #logs == 0 then
        TextLabel.Text = "[GazzHook] Chưa có log nào!"
        return
    end
    TextLabel.Text = "[GazzHook] Đang lưu tất cả..."
    spawn(function()
        local allCode = "-- ═══════════════════════════════════════\n"
            .. "-- GazzHook - Save All Remotes\n"
        ...
        local filename = GAZZHOOK_FOLDER .. "//SaveAll_" .. getStamp() .. ".lua"
        xpcall(function()
            writefile(filename, allCode)
            TextLabel.Text = "[GazzHook] Đã lưu " .. saved .. "/" .. #logs .. " remotes!"
        end, function(err)
            TextLabel.Text = "[GazzHook] Lỗi ghi file: " .. tostring(err)
        end)
    end)
end)

--- [GazzHook] FuncStore: Lưu Function Info của remote đang chọn vào GazzHook/FuncStore/
newButton("FuncStore", function()
    return "[GazzHook] Lưu Function Info vào GazzHook/FuncStore/\nDùng để lưu toàn bộ info của function tìm được"
end, function()
    if not (writefile and isfolder) then
        TextLabel.Text = "[GazzHook] Executor không hỗ trợ writefile!"
        return
    ...
    local func = selected and selected.Function
    if not func then
        TextLabel.Text = "[GazzHook] Không tìm thấy function!"
        return
    end
    spawn(function()
        local funcInfo = ""
        ...
        local filename = GAZZHOOK_FUNC_FOLDER .. "//" .. remoteName .. "_FuncInfo_" .. getStamp() .. ".lua"
        local content = "-- ═══════════════════════════════════════\n"
            .. "-- GazzHook FuncStore\n"
            .. "-- Remote: " .. tostring(remoteName) .. "\n"
            .. "-- ═══════════════════════════════════════\n\n"
            .. "-- Calling function info\n"
            .. "-- Generated by GazzHook FuncStore\n\n"
            .. funcInfo

        xpcall(function()
            writefile(filename, content)
            TextLabel.Text = "[GazzHook] FuncStore: Đã lưu " .. remoteName .. "!"
            -- Hiện lên codebox luôn
            codebox:setRaw(content)
        end, function(err)
            TextLabel.Text = "[GazzHook] Lỗi FuncStore: " .. tostring(err)
        end)
    end)
end)

--- [GazzHook] Hiện danh sách file đã lưu trong GazzHook
newButton("GazzFiles", function()
    return "[GazzHook] Xem danh sách file đã lưu trong thư mục GazzHook"
end, function()
    if not (listfiles and isfolder) then
        TextLabel.Text = "[GazzHook] Executor không hỗ trợ listfiles!"
        return
    end
    spawn(function()
        xpcall(function()
            local files = listfiles(GAZZHOOK_FOLDER)
            local funcFiles = isfolder(GAZZHOOK_FUNC_FOLDER) and listfiles(GAZZHOOK_FUNC_FOLDER) or {}
            local result = "-- ═══════════════════════════════════════\n"
                .. "-- GazzHook - Danh sách file đã lưu\n"
                .. "-- ═══════════════════════════════════════\n\n"
                .. "-- 📁 GazzHook/ (" .. #files .. " files)\n"
            for i, f in next, files do
                result = result .. "--   [" .. i .. "] " .. tostring(f) .. "\n"
            end
            result = result .. "\n-- 📁 GazzHook/FuncStore/ (" .. #funcFiles .. " files)\n"
            for i, f in next, funcFiles do
                result = result .. "--   [" .. i .. "] " .. tostring(f) .. "\n"
            end
            codebox:setRaw(result)
            TextLabel.Text = "[GazzHook] " .. (#files + #funcFiles) .. " file tổng cộng"
        end, function(err)
            TextLabel.Text = "[GazzHook] Lỗi đọc thư mục: " .. tostring(err)
        end)
    end)
end)
