...
[PANEL TRÁI - LOGS]
► Màu VÀNG = RemoteEvent (RE)
► Màu TÍM  = RemoteFunction (RF)
► Chữ nhỏ trên màu cho biết loại remote

[PANEL PHẢI - CODE]
► Hiển thị code được tạo tự động
...
💾 SaveAll   - Lưu tất cả logs ra file
             ...
             File: log_<tên remote>.lua

📋 Copy Code  - Copy code vào clipboard
📌 Copy Remote- Copy đường dẫn remote
▶  Run Code  - Chạy code trong codebox
📄 Get Script - Copy script gọi remote
🔍 Func Info - Xem info hàm gọi remote
🗑  Clr Logs  - Xóa toàn bộ logs

🚫 Exclude(i) - Ẩn remote này (theo instance)
🚫 Exclude(n) - Ẩn remote này (theo tên)
♻  Clr Blacklist - Xóa danh sách ẩn

⛔ Block(i)   - Chặn remote này (theo instance)
⛔ Block(n)   - Chặn remote này (theo tên)
♻  Clr Blocklist - Bỏ chặn tất cả

🔧 Disable Info  - Bật/tắt Function Info
🤖 Autoblock     - Tự động chặn spam
📜 Decompile     - Decompile script nguồn
🔑 CallingScript  - Dùng getcallingscript()
🔑 KeyToString   - Chuyển key thành string
📊 ToggleReturnValues - Ghi lại return values
📊 GetReturnValue - Xem return value
...
► Mỗi remote được lưu thành 1 file .lua
► Tên file: log_<tên remote>_<số>.lua

[GHI CHÚ]
► GazzSpy chỉ chạy được ở Client
► Cần exploit hỗ trợ hookfunction
► Autoblock giúp lọc remote spam
]]
...
newButton("SaveAll", function()
	...
	if not (writefile and makefolder and isfolder) then
		TextLabel.Text = "Exploit không hỗ trợ writefile/makefolder!"
		return
	end
	-- Tạo thư mục GazzSpy và FuncStore
	...
	for i, log in pairs(logs) do
		...
		if script ~= "" then
			...
			pcall(function()
				writefile(fileName, "-- GazzSpy Log | Remote: " .. tostring(log.Name) .. " | Type: " .. tostring(log.Type) .. "\n\n" .. script)
				savedCount = savedCount + 1
			end)
		end
	end
	TextLabel.Text = "SaveAll: Đã lưu " .. tostring(savedCount) .. " file vào GazzSpy/FuncStore/"
end)

-- ===== CÁC NÚT CHỨC NĂNG GỐC =====

newButton("Copy Code", function()
	return "Click để copy code vào clipboard"
end, function()
	setclipboard(codebox:getString())
	TextLabel.Text = "Đã copy thành công!"
end)

newButton("Copy Remote", function()
	return "Click để copy đường dẫn của remote"
end, function()
	if selected then
		setclipboard(v2s(selected.Remote.remote))
		TextLabel.Text = "Đã copy!"
	end
end)

newButton("Run Code", function()
	return "Click để chạy code trong codebox"
end, function()
	...
	else TextLabel.Text = "Lỗi khi chạy!" end
end)

newButton("Get Script", function()
	return "Click để copy script đã gọi remote\nCẢNH BÁO: Không phải lúc nào cũng chính xác"
end, function()
	if selected then
...
newButton("Function Info", function()
	return "Click để xem thông tin hàm đã gọi remote"
end, function()
	if selected then
		if selected.Function then
			codebox:setRaw("-- Calling function info\n-- Generated by GazzSpy\n\n" .. tostring(selected.Function))
		end
		TextLabel.Text = "Xong! Function info được tạo bởi GazzSpy."
	end
end)

newButton("Clr Logs", function()
	return "Click để xóa toàn bộ logs"
end, function()
...
newButton("Exclude (i)", function()
	return "Click để ẩn remote này (theo instance).\nRemote vẫn hoạt động nhưng không hiện trong logs."
end, function()
	if selected then blacklist[selected.Remote.remote] = true; TextLabel.Text = "Đã ẩn!" end
end)

newButton("Exclude (n)", function()
	return "Click để ẩn tất cả remote cùng tên.\nRemote vẫn hoạt động nhưng không hiện trong logs."
end, function()
	if selected then blacklist[selected.Name] = true; TextLabel.Text = "Đã ẩn!" end
end)

newButton("Clr Blacklist", function()
	return "Click để xóa danh sách ẩn remote."
end, function()
	blacklist = {}; TextLabel.Text = "Đã xóa blacklist!"
end)

newButton("Block (i)", function()
	return "Click để chặn remote này không gửi lên server (vẫn hiện trong logs)."
end, function()
	if selected then
		if selected.Remote.remote then blocklist[selected.Remote.remote] = true; TextLabel.Text = "Đã chặn!"
		else TextLabel.Text = "Lỗi! Instance có thể đã bị xóa." end
	end
end)

newButton("Block (n)", function()
	return "Click để chặn tất cả remote cùng tên không gửi lên server."
end, function()
	if selected then blocklist[selected.Name] = true; TextLabel.Text = "Đã chặn!" end
end)

newButton("Clr Blocklist", function()
	return "Click để bỏ chặn tất cả remote."
end, function()
	blocklist = {}; TextLabel.Text = "Đã xóa blocklist!"
end)

newButton("Decompile", function()
	return "Cố gắng decompile script nguồn\nCẢNH BÁO: Không phải lúc nào cũng thành công"
end, function()
	if selected then
		if selected.Source then codebox:setRaw(decompile(selected.Source)); TextLabel.Text = "Xong!"
		else TextLabel.Text = "Không tìm thấy source!" end
	end
end)

newButton("Disable Info", function()
	return string.format("[%s] Bật/tắt Function Info (có thể gây lag)", funcEnabled and "BẬT" or "TẮT")
end, function()
	funcEnabled = not funcEnabled
	TextLabel.Text = string.format("[%s] Function Info", funcEnabled and "BẬT" or "TẮT")
end)

newButton("Autoblock", function()
	return string.format("[%s] [BETA] Tự động chặn remote spam", autoblock and "BẬT" or "TẮT")
end, function()
	autoblock = not autoblock
	TextLabel.Text = string.format("[%s] Autoblock", autoblock and "BẬT" or "TẮT")
	history = {}; excluding = {}
end)

newButton("CallingScript", function()
	return string.format("[%s] [UNSAFE] Dùng getcallingscript() để lấy script chính xác hơn (có thể bị detect)", useGetCallingScript and "BẬT" or "TẮT")
end, function()
	useGetCallingScript = not useGetCallingScript
	TextLabel.Text = string.format("[%s] CallingScript", useGetCallingScript and "BẬT" or "TẮT")
end)
...
newButton("KeyToString", function()
	...
	TextLabel.Text = string.format("[%s] KeyToString", keyToString and "BẬT" or "TẮT")
end)
...
newButton("ToggleReturnValues", function()
	...
	TextLabel.Text = string.format("[%s] Record Return Values", recordReturnValues and "BẬT" or "TẮT")
end)

newButton("GetReturnValue", function()
	return "[Experimental] Xem return value của RemoteFunction (nếu đã bật ToggleReturnValues)"
end, function()
